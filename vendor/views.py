    ...
    def perform_create(self, serializer):
        ...
        for key,value in self.request.data.items():
            if key.startswith('specifications') and '[title]'in key:
                ...
                specificaion_data.append({'title':title, 'content':content})
        ...
        self.save_nasted_data(product_instance,SpecificationSerializer,specificaion_data)
        self.save_nasted_data(product_instance,ColorSerializer,colors_data)
        self.save_nasted_data(product_instance,SizeSerializer,sizes_data)
        self.save_nasted_data(product_instance,GallerySerializer,gallery_data)
    
    def save_nasted_data(self,product_instance,serializer_class,data):
        serializer=serializer_class(data=data,many=True,context={'product_instance':product_instance})
        serializer.is_valid(raise_exception=True)
        serializer.save(product=product_instance)          
class ProducUpdateView(generics.RetrieveUpdateAPIView):
    serializer_class=ProductSerializer
    queryset=Product.objects.all()
    ...
    def update(self,request, *args, **kwargs):
        ...
        for key,value in self.request.data.items():
            if key.startswith('specifications') and '[title]'in key:
                ...
                image=value
                gallery_data.append({'image':image})
        self.save_nasted_data(product,SpecificationSerializer,specificaion_data)
        self.save_nasted_data(product,ColorSerializer,colors_data)
        self.save_nasted_data(product,SizeSerializer,sizes_data)
        self.save_nasted_data(product,GallerySerializer,gallery_data)
    
    def save_nasted_data(self,product,serializer_class,data):
